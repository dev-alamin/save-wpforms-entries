...
<div x-data="migrationHandler()" x-init="init()"
    ...
    <div class="bg-indigo-50 border border-indigo-200 p-6 rounded-lg space-y-4">
        ...
        <div class="bg-white rounded-md border border-gray-200 shadow-sm p-4">
            <h2 class="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                ...
                <?php esc_html_e('Migration Overview', 'advanced-entries-manager-for-wpforms'); ?>
            </h2>
        ...
        <template x-if="totalEntries > 10000">
            <p class="text-sm text-red-600">
                <?php esc_html_e('⚠️ You have a large dataset. We recommend using a batch size of 100 or less to avoid timeouts.', 'advanced-entries-manager-for-wpforms'); ?>
            </p>
        </template>

        <div class="flex items-center gap-4">
            <span class="text-gray-700 text-sm"><?php esc_html_e('Batch Size:', 'advanced-entries-manager-for-wpforms'); ?></span>
            <template x-if="totalEntries > 10000">
                <label class="block">
                    <div class="relative mt-1 w-32">
                        <input type="number" min="10" max="100" x-model.number="batchSize"
                            ...
                            placeholder="<?php echo esc_attr__('50', 'advanced-entries-manager-for-wpforms'); ?>"
            ...
            <button @click="seeProgress"
                title="<?php esc_attr_e('View Migration Progress', 'advanced-entries-manager-for-wpforms'); ?>"
                x-show="!migrating && !complete && migrationInProgress"
                ...
                <?php esc_html_e('Migration is running. See Progress', 'advanced-entries-manager-for-wpforms'); ?>
            </button>
    ...
    <div x-show="migrating" class="mt-4 space-y-2 bg-indigo-50 p-6 pb-8 rounded-lg">
        <h2 class="text-lg font-semibold mb-2"><?php esc_html_e('Migration Progress', 'advanced-entries-manager-for-wpforms'); ?></h2>
        <!-- Label -->
        <p class="text-sm text-gray-700 font-medium">
            <?php esc_html_e('Migrating Entries:', 'advanced-entries-manager-for-wpforms'); ?> <span class="font-bold" x-text="migrated"></span> /
            <span x-text="total"></span>
        </p>

        <!-- Estimated Time -->
        <p class="text-sm text-gray-500 italic" x-show="estimatedTime">
            <?php esc_html_e('Estimated time left:', 'advanced-entries-manager-for-wpforms'); ?> <span x-text="estimatedTime"></span>
        </p>

        <!-- Progress Label -->
        <div class="mb-1 text-sm text-gray-700 font-medium flex justify-between">
            <span><?php esc_html_e( 'Progress:', 'advanced-entries-manager-for-wpforms' ) ?></span>
