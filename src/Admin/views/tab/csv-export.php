...
" class="bg-white border border-indigo-100 shadow p-6 space-y-6 rounded-lg">
    ...
    <!-- Batch Size Input -->
    <div>
        <label for="swpfe_export_limit" class="block text-sm font-semibold text-gray-700 mb-1">
        ...
        <input type="number" name="swpfe_export_limit" id="swpfe_export_limit"
            value="<?php echo esc_attr(get_option('swpfe_export_limit', 500)); ?>"
            min="10" max="5000"
            class="!w-full !px-4 !py-2 !border !border-gray-300 !rounded-md !shadow-sm !text-sm !focus:ring !focus:ring-indigo-200 !focus:border-indigo-500">
        <p class="text-xs text-gray-500 mt-1">
            <?php esc_html_e('Used to process large exports in chunks. Default is 500.', 'advanced-entries-manager-for-wpforms'); ?>
        </p>
    </div>
    ...
    <div x-show="fields.length > 0">
        <label class="block text-sm font-semibold text-gray-700 mb-2">
            <?php esc_html_e('Exclude Columns (Optional)', 'advanced-entries-manager-for-wpforms'); ?>
        </label>
    ...
    <div
        ...
        <div @click.away="closeProgressModal()" class="bg-white rounded-lg shadow-xl p-6 w-96 max-w-full">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800"><?php esc_html_e('Export Progress', 'advanced-entries-manager-for-wpforms'); ?></h3>
