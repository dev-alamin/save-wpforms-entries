...
use App\AdvancedEntryManager\Admin\Pages\LogViewerPage;
...
class HandleLogAction {
	...
	protected function handle_download() {
		...
		if ( $this->fs->exists( $file_path ) && strpos( realpath( $file_path ), realpath( $log_dir ) ) === 0 ) {
			...
			$redirect_url = add_query_arg( array( 'message' => urlencode( 'File not found or invalid.' ) ), admin_url( 'admin.php?page=forms-entries-manager-logs' ) );
